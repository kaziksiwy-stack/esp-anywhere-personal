...
5. **Autoryzacja w Transporcie**: Token jest przekazywany wyłącznie w nagłówku HTTP `Authorization: Bearer <token>` przy nawiązywaniu WebSocketa. Urządzenie bez ważnego tokena nie zbuduje połączenia.
...
14. **OTA niedostępne w firmware prototypowym**: klient ESP32 jawnie odrzuca `install_update`; bezpieczne OTA wymaga osobnego, kompletnego wdrożenia weryfikacji podpisu i nie jest deklarowane jako działające.
